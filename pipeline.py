...
try:
    ...
    logger.info(f"Combined dataframe now has {len(combined_df)} rows after deduplication.")
...
try:
    ...
    form_responses_df = gsheets.read_from_gsheets("Form Responses", GOOGLE_SHEETS_URL_KEY)
...
try:
    ...
    if alert['issue_alert']:
        logger.info(f"Issue detected! Sending Pushover notification...")
